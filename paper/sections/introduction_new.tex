...
uploading datasets to ChatGPT (and similar platforms) and typing requests like ``compare last
...
The results are concerning, at least in part. The model generates a \proglang{Python}
...
architecture is not.\footnote{We do not claim that data analysis is the main purpose of ChatGPT and similar applications; however, it has become a widely adopted use case---as evidenced by OpenAI's prominent integration of a data analysis mode---making the architectural shortcomings particularly consequential for applied researchers.} These problems are not inherent to
conversational data analysis; they arise from a specific design
choice: delegating the \emph{orchestration} of computation to generated
code. The generated script typically calls established libraries
(\pkg{statsmodels}, \pkg{pandas}), so the underlying algorithms are
sound---but the model must still select the right method, pass the
correct parameters, and assemble the pipeline without silent errors,
all without any built-in safeguard that it has done so correctly.
...
data analytics library in pure \proglang{Rust}---chosen for its
memory safety without garbage collection, which enables predictable
performance and straightforward compilation to single-binary
deployments---exposed through the
...
discrete choice, time series, and over 200 additional methods---validated
both by Monte Carlo simulation of statistical properties (coverage rates,
Type~I error, standard error calibration) and by numerical comparison with
established \proglang{R} packages.
...
benchmark scripts for both \proglang{R} and \proglang{Rust}, the Monte
Carlo validation pipeline, and validation test suites---are located in
the repository's \code{paper/} directory.
...
examples including iterative multi-step workflows.
