...
scenarios, standard error calibration and Type~I error control are
exact: all SE accuracy tests pass (mean SE ratio 1.005) and all
Type~I error tests pass.  Coverage deviations occur in panel FE/RE
with few clusters ($G = 14$): entity-clustered SEs are well
calibrated, but normal-approximation confidence intervals undercover
(91\% vs.\ 95\%) because the $t$-distribution with $G - 1 = 13$
degrees of freedom is more appropriate than $z_{0.025} = 1.96$ in
this regime---a known small-sample issue with cluster-robust
inference~\citep{Cameron+Miller:2015}.  IV/2SLS shows slightly
conservative coverage (97.6\%), consistent with finite-sample
properties of 2SLS. Full results and the simulation code are included
