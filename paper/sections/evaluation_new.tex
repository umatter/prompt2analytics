...
Table~\ref{tab:confusion-patterns} summarizes the most frequent
misselection patterns from the naturalistic prompt evaluation (891
test instances across 9~models).  Four categories emerge.
\emph{Related test substitution} (e.g., ADF $\to$ Phillips-Perron)
selects a valid alternative from the same test family---often
harmless, but not what was requested.  \emph{Specific $\to$ omnibus}
maps a named diagnostic (Breusch-Pagan, VIF) to the general
diagnostics bundle, losing precision.  \emph{Method-level confusion}
produces a fundamentally different estimator (NLS~$\to$~OLS,
hierarchical clustering~$\to$~DBSCAN); the ``clustered~SEs $\to$
K-means'' case illustrates how lexical overlap in the word
``cluster'' misleads smaller models.  \emph{Scope confusion} selects
a simpler variant that omits a requested feature (event study $\to$
basic DiD, two-way FE $\to$ one-way FE).

\begin{table}[htbp]
\centering
\begin{threeparttable}
\caption{Most frequent tool misselection patterns (naturalistic prompts)}
\label{tab:confusion-patterns}
\small
\begin{tabular}{llrl}
\toprule
Expected & Selected & $n$ & Category \\
\midrule
ADF test & Phillips-Perron & 17 & Related test \\
Breusch-Pagan & Diagnostics bundle & 16 & Specific $\to$ omnibus \\
Auto-ARIMA & AR & 15 & Method confusion \\
Durbin-Watson & Box-Ljung & 10 & Related test \\
Two-way FE & One-way FE & 9 & Scope confusion \\
Hierarchical & DBSCAN & 9 & Method confusion \\
NLS & OLS & 8 & Method confusion \\
Welch $t$ & Standard $t$ & 8 & Related test \\
VIF & Diagnostics bundle & 8 & Specific $\to$ omnibus \\
Histogram & Describe dataset & 8 & Scope confusion \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
\item \emph{Note:} Counts aggregated across 9~models on 99
naturalistic prompts (891 model--prompt pairs). Naming aliases
(e.g., \code{panel\_fe} vs.\ \code{panel\_fixed\_effects})
excluded. Full scored results are in the supplementary materials.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
These observations suggest that for curated prompts, the dominant
failure mode is tool misselection (5--10\% of requests), not
parameter error. For naturalistic prompts, where tool accuracy drops
to 58--70\%, both misselection and ambiguous parameter specification
contribute. In all cases, users can inspect the tool call before
execution, correct misselected methods or missing parameters, and
iterate.
...
methods with more than 1,500 individual test cases (334 specifically
targeting validation against \proglang{R} reference implementations).
...
methods (VAR, ARIMA) match \pkg{vars} and \pkg{forecast}. The \pkg{p2a-core} test suite comprises 1{,}962 test functions.
...
validation breadth---each implemented method has at least one
cross-validated test against an \proglang{R} reference.  The CI pipeline does \emph{not} execute the
...
output values. In total, 55 R reference scripts (one per validated
method) and the Rust benchmarking runner are provided in the
...
LTMLE, CBPS, and entropy balancing---have internal consistency tests
(correct output dimensions, convergence on simulated DGPs, coefficient
sign verification, weight-sum constraints) but lack direct numerical
cross-validation against \proglang{R} reference packages.  The
corresponding \proglang{R} implementations (e.g., \pkg{did},
\pkg{DoubleML}, \pkg{Synth}, \pkg{CBPS}) use different default tuning
parameters, cross-validation splits, and optimization algorithms that
complicate exact numerical comparison; matching output requires careful
configuration alignment that is not yet in place.  Extending
cross-validation coverage to these methods is a priority for future
releases.
