...
This appendix provides detailed single-turn tool selection results from
the evaluation described in Section~\ref{sec:evaluation}.

\subsection{Single-turn tool selection}

Table~\ref{tab:model-summary} presents accuracy, cost, and failure rates
across all evaluated models. Table~\ref{tab:category-accuracy} breaks down
...
\noindent The key pattern across both tables is that all models exceed 90\%
accuracy, with Ministral 3B the lowest at 90.8\%.
Accuracy drops concentrate in hypothesis testing and time series
categories, where distinguishing between closely related methods
(e.g., VAR vs.\ VECM) proves challenging. Cost per query varies
by two orders of magnitude across providers.
...
\caption{API response latency by model (log scale). Latencies reflect
cloud-hosted inference including network round-trip time; local
deployment would differ substantially.}
...
on model size and system specifications. For interactive use,
GPU-accelerated inference with a 7--8B model provides sub-second
tool selection, comparable to cloud API latency.
...
Server (large data) & Qwen 2.5 72B & 128 GB & $2 \times 2$4 GB & 60 GB \\
\bottomrule
\end{tabular}
\begin{tablenotes}[flushleft]
...
\noindent For the simplest setup, the Docker deployment
(the Docker deployment subsection of Appendix~\ref{app:deployment}) bundles all dependencies including
...
A time series workflow using quarterly U.S.\ GDP growth data
(1954--2023) illustrates method progression within a single
conversation: stationarity testing, univariate modeling, then
