...
Diagnostic tests are validated against \pkg{lmtest}:

\begin{itemize}
  \item \textbf{Jarque-Bera}: Normality test statistic matches to $< 10^{-4}$
  \item \textbf{Breusch-Pagan}: Heteroskedasticity test matches to $< 10^{-4}$
  \item \textbf{Durbin-Watson}: Autocorrelation statistic matches to $< 10^{-6}$
  \item \textbf{VIF}: Variance inflation factors match to $< 10^{-6}$
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Default robust SE type}: \pkg{prompt2analytics} defaults to
    HC1 (consistent with Stata's \code{robust} option), while \proglang{R}'s
    \pkg{sandwich} defaults to HC3.

  \item \textbf{Coefficient naming}: \proglang{R} uses variable names in output;
    \pkg{prompt2analytics} uses numeric indices internally but displays names
    in formatted output.

  \item \textbf{$R^2$ variants}: For panel models, we report ``within $R^2$''
    by default, matching \pkg{plm}'s convention for fixed effects models.

  \item \textbf{Convergence tolerance}: Iterative methods (HDFE, MLE) use
    a default tolerance of $10^{-8}$, which may differ slightly from
    \proglang{R} package defaults.
\end{enumerate}

These differences are documented in the method documentation and do not
affect the validity of statistical inference.
