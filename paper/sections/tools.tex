...
for numerically stable linear algebra. Implementations follow standard
textbook algorithms: OLS and robust standard errors follow
\citet{Wooldridge:2010} and \citet{Cameron+Trivedi:2005}; panel data
methods follow \citet{Baltagi:2013}; time series methods follow
\citet{Hamilton:1994}; maximum likelihood for discrete choice uses
Newton-Raphson as described in \citet{Greene:2018}. An optional
...
transports; and \pkg{p2a-dioxus} provides a web application using
Dioxus that compiles to WebAssembly for browser deployment. All three
...
\caption{Software architecture of \pkg{prompt2analytics}. The CLI
accesses the analytics engine directly (orange); chat interfaces route
through the LLM layer and MCP server (cyan). All computation occurs
locally in \pkg{p2a-core}.}
...
\noindent For LLM-mediated access, configure the MCP server and type
in natural language:
...
\caption{Interaction flow from user request to result. The LLM
generates a structured tool call; the MCP server executes the
\proglang{Rust} method; results flow back for explanation. Data
processing (shaded) stays local.}
\label{fig:chat-flow}
\end{figure}
...
Each tool invokes a fixed \proglang{Rust} function validated against
reference \proglang{R} implementations
...
\noindent Implementors provide core results; derived statistics
($t$-values, $p$-values, confidence intervals) are computed by default
implementations, enforced at compile time. The \code{LinearEstimator}
trait applies only to linear models (OLS, GLS, panel FE/RE). Non-linear
models use their own result types: discrete choice models
(\code{DiscreteResult}) return $z$-statistics, log-likelihood, and
pseudo-$R^2$ rather than $t$-statistics and residuals; panel models
(\code{PanelResult}) include within-$R^2$ and group structure; time
series models return information criteria and forecast output;
clustering methods (\code{ClusterResult}) return cluster assignments,
centroids, and fit measures (silhouette, inertia); and survival models
(\code{SurvivalResult}) return hazard ratios, survival curves, and
log-rank statistics. Each result type implements a common serialization
interface so the MCP layer can return structured output regardless of
the method family. Unlike \proglang{R}'s
...
When intentional differences exist---for example, defaulting to HC1
rather than HC3 for robust standard errors, matching \proglang{Stata}'s
convention---these are documented and tested separately. The validation
...
The \pkg{p2a-core} library implements over 250 methods, exposed as
...
\subsection{The MCP tool layer}

The MCP server exposes tools organized into categories that mirror
the core library (Table~\ref{tab:mcp-tools}).
...
The full set of tool schemas consumes approximately 32{,}000 tokens,
a substantial share of current context windows.  The built-in
chat interface therefore exposes a curated subset of
roughly 70 tools covering the most common analytical workflows;
specialized tools remain available through the CLI.
External MCP clients receive the complete schema and can apply
their own filtering.

\paragraph{Multi-turn context management.}
In multi-turn conversations, the system includes prior tool calls and
their results in the conversation history sent to the LLM. Each
assistant message that triggered a tool call is preserved with its
structured \code{tool\_calls} field, followed by a \code{tool} role
message containing the result (truncated to 2{,}000 characters to
prevent context overflow). This enables the LLM to resolve references
like ``those results'' or ``now try with clustered standard errors''
by inspecting what was previously computed.  A token budget tracker
estimates context usage across the system prompt, conversation history,
tool schemas, and the current query; when the budget approaches the
model's context window, older turns are summarized (retaining tool
names and key results) while recent turns are kept verbatim.

For the
curated 70-tool schema (${\approx}\,12{,}000$ tokens), a 128K-context
model supports roughly 30--40 substantive turns before summarization
begins; the full 250+ tool schema (${\approx}\,32{,}000$ tokens) reduces
this to approximately 20 turns. External MCP clients (e.g., Claude
Desktop, Cursor) manage their own context windows and may apply
different truncation or tool-filtering strategies.
...
automatically. For matching and IPW, balance diagnostics flag
...
The choice of \proglang{Rust} follows from the principles.
Principle~2 (pre-validation) requires an implementation language where
validated methods remain correct across deployments---\proglang{Rust}'s
compile-time memory safety guarantees eliminate buffer overflows,
use-after-free, and data races that can cause incorrect results in
numerical software. Principle~3 (data locality) benefits from compiled
performance: the same analytics engine runs efficiently on the user's
machine, not just on cloud servers. A single codebase compiles to
standalone binaries, WebAssembly for browsers, and shared libraries
