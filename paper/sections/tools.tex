...
The full MCP server exposes all methods as individual tools;
after consolidation (e.g., one \code{regression\_ols} tool covering
HC0--HC3, HAC, and bootstrap), this yields approximately 250 MCP tools
whose JSON schemas consume ${\approx}\,32{,}000$ tokens---a
substantial share of current context windows.  The built-in
chat interface therefore exposes a curated subset of
roughly 70 tools covering the most common analytical workflows;
the evaluation in Section~\ref{sec:evaluation} uses a broader
set of 119~Tier-1 tools.  All tools remain accessible through the
CLI and external MCP clients, which receive the complete schema and
can apply their own filtering.
...
begins; the full ${\approx}\,250$-tool schema (${\approx}\,32{,}000$ tokens) reduces
