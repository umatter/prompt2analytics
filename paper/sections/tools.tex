...
implementations, enforced at compile time. Other method families use
their own result types: discrete choice models return $z$-statistics
and log-likelihood rather than $t$-statistics and residuals; panel
models include within-$R^2$ and group structure; time series models
include information criteria and forecast output. Unlike \proglang{R}'s
formula interface, the API uses explicit column names, because the LLM
has already translated intent into specification.
...
The \pkg{p2a-core} library implements 271 methods, exposed as
...
a substantial share of current context windows.  For the built-in
chat interface, the MCP server therefore exposes a curated subset of
roughly 70 tools covering the most common analytical workflows;
specialized tools are available on request or through the CLI.
External LLM clients that connect via MCP receive the complete
schema and can apply their own filtering.
...
date, no econometrics or data analytics library has been available for \proglang{Rust};
