...
\section{Defining chat-first analytics} \label{sec:defining}
...
to analyze a dataset, the data is uploaded to a third-party server
...
(Section~\ref{sec:evaluation}) shows that even 3-billion-parameter
models achieve over 90\% tool selection accuracy, making fully local
...
Hybrid designs are possible. A system could generate code for bespoke
analyses while routing standard requests to pre-validated tools, or
could verify generated code through static analysis before execution.
\pkg{prompt2analytics} deliberately occupies one end of this spectrum:
all computation is pre-validated, with no code generation at all. This
sacrifices flexibility (analyses outside the method library are
unavailable) but eliminates an entire class of failure modes. Whether
hybrid designs can achieve a better trade-off is an open question.
...
Two limitations of the approach are fundamental, not fixable by
...
than reporting unavailability. This is the symmetric failure mode to
