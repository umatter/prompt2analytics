...
use crate::regression::ols::{run_ols, OlsResult, CovarianceType};
...
pub fn step(
    ...
    for _ in 0..config.max_steps {
        ...
        // Find best move
        let (best_action, best_new_vars, best_new_criterion, best_new_model) =
            find_best_move(
                dataset,
                y_col,
                &current_vars,
...
fn find_best_move(
    ...
    if can_drop {
        for var in current_vars {
            ...
            let new_vars: Vec<String> = current_vars
                .iter()
                .filter(|v| *v != var)
                .cloned()
                .collect();
    ...
    if best_model.is_none() {
        ...
        return Ok((None, current_vars.to_vec(), current_criterion, current_model));
    }
...
pub fn add1(
    ...
    // Fit current model
    let current_model = run_ols(dataset, y_col, current_vars, intercept, CovarianceType::Standard)?;
    ...
    for &var in candidates {
        ...
        if let Ok(model) = run_ols(dataset, y_col, &new_vars, intercept, CovarianceType::Standard) {
            let criterion = compute_criterion(&model, k);
...
mod tests {
    ...
    fn create_test_dataset() -> Dataset {
        ...
        let y: Vec<f64> = x1.iter()
            ...
            .enumerate()
            .map(|(i, (&x1i, &x2i))| {
                2.0 * x1i + 3.0 * x2i + (i as f64 * 0.01).sin() * 0.5
            })
            .collect();
