...
impl ReportSection {
    ...
    /// Add a chart image.
    pub fn add_chart(&mut self, image_base64: impl Into<String>, title: Option<&str>, caption: Option<&str>) {
...
fn render_content(content: &ReportContent) -> String {
    match content {
        ...
        ReportContent::Chart { title, image_base64, caption } => {
            let mut html = String::from("            <figure class=\"chart\">\n");
            if let Some(t) = title {
                html.push_str(&format!("                <figcaption class=\"chart-title\">{}</figcaption>\n", escape_html(t)));
            }
            html.push_str(&format!(
...
fn simple_markdown_to_html(md: &str) -> String {
    ...
    for line in md.lines() {
        ...
        // Bold
            ...
            let processed = line.replace("**", "<strong>").replace("<strong>", "</strong>");
