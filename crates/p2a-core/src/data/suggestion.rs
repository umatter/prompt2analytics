...
use super::quality::{DataQualityProfile, DataIssue, ColumnProfile};
...
pub fn generate_suggestions(profile: &DataQualityProfile) -> SuggestionReport {
    ...
    for col_profile in &profile.columns {
        for issue in &col_profile.issues {
            if let Some(suggestion) = suggestion_for_column_issue(issue, col_profile, profile, &mut next_id) {
                suggestions.push(suggestion);
            }
        }
    }
    ...
    suggestions.sort_by(|a, b| {
        b.priority.cmp(&a.priority)
            .then_with(|| b.estimated_impact.rows_affected.cmp(&a.estimated_impact.rows_affected))
    });
    ...
    let issues_analyzed = profile.dataset_issues.len()
        + profile.columns.iter().map(|c| c.issues.len()).sum::<usize>();
    ...
    match issue {
        ...
        DataIssue::PossibleDuplicates { columns, count } => {
            ...
            Some(CleaningSuggestion {
                ...
                reasoning: "These rows share the same values in key columns and may represent duplicates.".to_string(),
                considerations: vec![
    ...
    match issue {
        DataIssue::HighNullRate { column, pct } => {
            ...
            Some(CleaningSuggestion {
                ...
                description: format!(
                    ...
                    column, pct * 100.0, null_count,
                    if *pct > 0.5 { "Consider dropping this column or investigating why so many values are missing." }
                    else { "Fill missing values or drop affected rows." }
                ),
                addresses_issue: issue.description(),
                priority,
                category: CleaningCategory::MissingValues,
                columns: Some(vec![column.clone()]),
                operation: if *pct > 0.7 { "drop_column".to_string() } else { "fill_na".to_string() },
                parameters: SuggestionParameters {
                ...
                estimated_impact: EstimatedImpact {
                    ...
                    impact_description: format!("Will affect {} rows ({:.1}%)", null_count, pct * 100.0),
                },
                reasoning,
                considerations: vec![
                    "Missing values may be informative (not missing at random)".to_string(),
                    "Consider the impact on downstream analysis".to_string(),
                    if *pct > 0.3 { "High null rate may indicate a data collection issue".to_string() } else { String::new() },
        ...
        DataIssue::InconsistentFormat { column, patterns } => {
            Some(CleaningSuggestion {
                ...
                reasoning: "Mixed formats can cause issues with parsing, sorting, and grouping operations.".to_string(),
                considerations: vec![
        ...
        DataIssue::ConstantColumn { column, value } => {
            Some(CleaningSuggestion {
                ...
                reasoning: "Constant columns add no variance and can be removed to simplify the dataset.".to_string(),
                considerations: vec![
    ...
    if let Some(ref numeric_stats) = col_profile.numeric_stats {
        ...
        if let Some((most_common, _)) = string_stats.top_values.first() {
            ...
            (
                ...
                "For string data with no clear mode, consider filling with a placeholder value.".to_string(),
            )
        }
    } else {
        (
            "drop".to_string(),
            None,
            "For this column type, dropping rows with missing values may be the safest option.".to_string(),
        )
    }
}

/// Generate an overall recommendation based on suggestions.
fn generate_overall_recommendation(suggestions: &[CleaningSuggestion], profile: &DataQualityProfile) -> String {
    if suggestions.is_empty() {
        return "Your dataset appears to be clean! No immediate cleaning actions are recommended.".to_string();
    }

    let critical_count = suggestions.iter().filter(|s| s.priority == SuggestionPriority::Critical).count();
    let high_count = suggestions.iter().filter(|s| s.priority == SuggestionPriority::High).count();
...
impl SuggestionReport {
    ...
    pub fn summary(&self) -> String {
        ...
        if self.suggestions.is_empty() {
            ...
            for priority in [SuggestionPriority::Critical, SuggestionPriority::High,
                ...
                let priority_suggestions: Vec<_> = self.suggestions.iter()
                    .filter(|s| s.priority == priority)
                    .collect();

                if !priority_suggestions.is_empty() {
                    summary.push_str(&format!("\n{} Priority ({}):\n", priority.label(), priority_suggestions.len()));
    ...
    pub fn by_priority(&self, priority: SuggestionPriority) -> Vec<&CleaningSuggestion> {
        self.suggestions.iter().filter(|s| s.priority == priority).collect()
    }
    ...
    pub fn for_column(&self, column: &str) -> Vec<&CleaningSuggestion> {
        self.suggestions.iter()
            .filter(|s| s.columns.as_ref().map_or(false, |cols| cols.contains(&column.to_string())))
            .collect()
    }
}
...
mod tests {
    ...
    fn test_suggestion_for_whitespace() {
        ...
        let trim_suggestion = report.suggestions.iter()
            .find(|s| s.operation == "trim");
    ...
    fn test_suggestion_for_nulls() {
        ...
        let null_suggestion = report.suggestions.iter()
    ...
    fn test_suggestion_for_duplicates() {
        ...
        let dedup_suggestion = report.suggestions.iter()
    ...
    fn test_suggestion_for_outliers() {
        ...
        let outlier_suggestion = report.suggestions.iter()
