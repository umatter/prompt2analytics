...
use p2a_core::{
    AcfType, Alternative, BoxTestType, Dataset, FisherAlternative, LoessConfig, NlsAlgorithm,
    NlsConfig, SpectrumConfig, TheoreticalDistribution, WilcoxonConfig, acf, bartlett_test,
    box_test, ccf, chisq_test_gof, chisq_test_independence, fisher_exact_test, ks_test_one_sample,
    ks_test_two_sample, loess, manova_one_way, model_exponential_decay, model_michaelis_menten,
    nls, one_sample_t_test, pacf, pp_test, run_diagnostics, run_ols_raw, run_one_way_anova,
    run_tukey_hsd, run_two_way_anova, shapiro_wilk_test, spectrum, spectrum_ar, two_sample_t_test,
    wilcoxon_rank_sum, wilcoxon_signed_rank,
};
use polars::prelude::*;
use rand::Rng;
...
fn generate_regression_data_raw(
    n: usize,
    k: usize,
    seed: u64,
...
fn ols_robust_se_benchmark(c: &mut Criterion) {
    ...
    for (name, cov_type) in [
        ...
        group.bench_with_input(BenchmarkId::from_parameter(name), &cov_type, |b, cov| {
            b.iter(|| run_ols_raw(&x, &y, &var_names, "y", *cov));
        });
    }
...
fn diagnostics_benchmark(c: &mut Criterion) {
    ...
    for n in [100, 1000, 10000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &dataset, |b, data| {
            b.iter(|| run_diagnostics(data, "y", &x_cols));
        });
    }
...
fn generate_two_way_anova_data(
    n_per_cell: usize,
    levels_a: usize,
    levels_b: usize,
    seed: u64,
...
fn generate_ttest_data(n: usize, seed: u64) -> (Vec<f64>, Vec<f64>) {
    ...
    let x: Vec<f64> = (0..n).map(|_| 5.0 + rng.gen_range(-1.0..1.0)).collect();
    let y: Vec<f64> = (0..n)
...
fn generate_categorical_data(n_categories: usize, total_count: usize, seed: u64) -> Vec<f64> {
    ...
    counts
        .iter_mut()
        .for_each(|c| *c = (*c / sum) * total_count as f64);

    counts
}

/// Generate synthetic contingency table for independence test
fn generate_contingency_table(
    n_rows: usize,
    n_cols: usize,
    total_count: usize,
    seed: u64,
    ...
    let mut table: Vec<Vec<f64>> = (0..n_rows)
        .map(|_| (0..n_cols).map(|_| rng.gen_range(1.0..100.0)).collect())
        .collect();
...
fn generate_exponential_decay_data(n: usize, seed: u64) -> (Array1<f64>, Array1<f64>) {
    ...
    let x: Array1<f64> = (0..n).map(|i| i as f64 * 5.0 / n as f64).collect();
...
fn generate_two_sample_data(n1: usize, n2: usize, seed: u64) -> (Vec<f64>, Vec<f64>) {
    let mut rng = ChaCha8Rng::seed_from_u64(seed);
    let x: Vec<f64> = (0..n1)
        .map(|_| rng.gen_range(0.0..1.0) * 10.0 + 5.0)
        .collect();
    let y: Vec<f64> = (0..n2)
        .map(|_| rng.gen_range(0.0..1.0) * 10.0 + 6.0)
        .collect();
    (x, y)
}
...
fn generate_paired_data(n: usize, seed: u64) -> (Vec<f64>, Vec<f64>) {
    let mut rng = ChaCha8Rng::seed_from_u64(seed);
    let x: Vec<f64> = (0..n)
        .map(|_| rng.gen_range(0.0..1.0) * 30.0 + 100.0)
        .collect();
    let y: Vec<f64> = x
        .iter()
        .map(|&xi| xi + rng.gen_range(-1.0..1.0) * 20.0 - 5.0)
        .collect();
    (x, y)
}
...
fn wilcoxon_rank_sum_benchmark(c: &mut Criterion) {
    ...
    for n in [10, 50, 100, 500, 1000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, _| {
            b.iter(|| wilcoxon_rank_sum(&x, &y, 0.0, Alternative::TwoSided, &config));
        });
    }
...
fn wilcoxon_signed_rank_benchmark(c: &mut Criterion) {
    ...
    for n in [10, 50, 100, 500, 1000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, _| {
            b.iter(|| wilcoxon_signed_rank(&x, Some(&y), 0.0, Alternative::TwoSided, &config));
        });
    }
...
fn generate_ks_two_sample_data(n: usize, seed: u64) -> (Vec<f64>, Vec<f64>) {
    ...
    let x: Vec<f64> = (0..n)
        ...
        .collect();
    let y: Vec<f64> = (0..n)
        .map(|_| rng.gen_range(0.0..1.0) * 2.0 - 0.9)
        .collect(); // Slightly shifted
    (x, y)
}
...
fn ks_two_sample_benchmark(c: &mut Criterion) {
    ...
    for n in [100, 1000, 10000, 100000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, _| {
            b.iter(|| ks_test_two_sample(&x, &y, Alternative::TwoSided));
        });
    }
...
fn ks_one_sample_benchmark(c: &mut Criterion) {
    ...
    for n in [100, 1000, 10000, 100000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, _| {
            b.iter(|| ks_test_one_sample(&x, dist, Alternative::TwoSided));
        });
    }
...
fn generate_manova_data(
    n_per_group: usize,
    n_groups: usize,
    n_vars: usize,
    seed: u64,
) -> (ndarray::Array2<f64>, Vec<String>) {
    use ndarray::Array2;
...
fn manova_benchmark(c: &mut Criterion) {
    ...
    for (n_per_group, n_groups, n_vars) in configs {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(&label), &label, |b, _| {
            b.iter(|| manova_one_way(&y, &groups));
        });
    }
...
fn tukey_hsd_benchmark(c: &mut Criterion) {
    ...
    for (n_per_group, n_groups) in configs {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(&label), &label, |b, _| {
            b.iter(|| run_tukey_hsd(&dataset, "y", "group", 0.95));
        });
    }
...
fn spectrum_periodogram_benchmark(c: &mut Criterion) {
    ...
    for n in [100, 1000, 10000, 100000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, _| {
            b.iter(|| spectrum(&x, SpectrumConfig::default()));
        });
    }
...
fn spectrum_ar_benchmark(c: &mut Criterion) {
    ...
    for n in [100, 1000, 10000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, _| {
            b.iter(|| spectrum_ar(&x, None, None));
        });
    }
...
fn box_test_ljung_box_benchmark(c: &mut Criterion) {
    ...
    for n in [100, 1000, 10000, 100000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, _| {
            b.iter(|| box_test(&x, Some(10), BoxTestType::LjungBox, 0));
        });
    }
...
fn box_test_box_pierce_benchmark(c: &mut Criterion) {
    ...
    for n in [100, 1000, 10000, 100000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, _| {
            b.iter(|| box_test(&x, Some(10), BoxTestType::BoxPierce, 0));
        });
    }
...
fn generate_loess_data(n: usize, seed: u64) -> (Vec<f64>, Vec<f64>) {
    let mut rng = ChaCha8Rng::seed_from_u64(seed);
    let x: Vec<f64> = (0..n)
        .map(|i| i as f64 / n as f64 * 4.0 * std::f64::consts::PI)
        .collect();
    let y: Vec<f64> = x
        .iter()
